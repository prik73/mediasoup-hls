...
export class SDPGenerator {
     ...
     */
    static async generateFile(consumers, ports, outputDir) {
     ...
     */
    static generateMultiStreamSDP(consumers, ports) {
        const sdpLines = [
            'v=0',
        ...
        for (let i = 0; i < consumers.length; i++) {
            ...
            if (consumer.videoConsumer) {
                ...
                sdpLines.push(`m=video ${port.video.rtp} RTP/AVP ${payloadType}`, `a=rtpmap:${payloadType} ${videoCodec.mimeType.split('/')[1]}/${videoCodec.clockRate}`, `a=fmtp:${payloadType} ${Object.entries(videoCodec.parameters || {}).map(([k, v]) => `${k}=${v}`).join(';')}`, `a=rtcp:${port.video.rtcp} IN IP4 127.0.0.1`);
            }
            // Audio media section
            if (consumer.audioConsumer) {
                ...
                sdpLines.push(`m=audio ${port.audio.rtp} RTP/AVP ${payloadType}`, `a=rtpmap:${payloadType} ${audioCodec.mimeType.split('/')[1]}/${audioCodec.clockRate}/${channels}`, `a=rtcp:${port.audio.rtcp} IN IP4 127.0.0.1`);
            }
        }
